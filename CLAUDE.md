...
This directory is a game. A terminal character (the guy) lives in a zellij pane and grows when his person merges or reviews pull requests. Forge runs are Claude Code sessions started here with a brief in `runs/*/forge-brief.md`. If you are one of those sessions, the brief you were given is the task. Nothing in this directory is off limits to you.

## Layout

- `guy` — the entry point. `./guy` runs the widget, `./guy boot-test` checks it boots.
- `termguy/` — the package. `ui.py` pages and loop, `mascot.py` the sprite and built-in idle jobs, `anim.py` registries items extend, `items.py` how manifests reach code, `world.py` zellij and the outside, `rules.py` growth arithmetic and seeded rolls, `sync.py` GitHub events to growth, `forge.py` runs briefs, `talents.py` the graph.
...
- Read `termguy/mascot.py` before drawing. Move the way he moves.
